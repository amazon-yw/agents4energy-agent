...
[Korean] 나는 풍력 발전소 단지를 관리하고 있으며, 운영비용을 최소화하면서 에너지 생산량을 극대화하고 싶습니다. 
이번 분기 우리 단지의 성과를 확인하고 싶습니다. 각 풍력 발전소의 실제 생산량과 예측 생산량을 비교해주세요. 
그리고 필요한 시계열 데이터를 생성하고 각 풍력 발전소의 성과를 시각화할 수 있는 대시보드를 렌더링해주세요.`,
...
[Korean] 샌후안 분지의 유정 압축기 단지에 대한 유지보수 일정을 최적화하는 데모를 만들어주세요. 
과거 유지보수 이벤트, 압축기 고장, 압축기 시계열 데이터가 포함된 최적화되지 않은 데이터를 생성해주세요. 
데이터를 분석하여 최적화된 유지보수 계획을 찾고 비용 절감 효과를 추정해주세요. 
종합적인 보고서를 작성하고 시각화를 위해 렌더링해주세요.`,
...
Create a comprehensive report and render it for visualization.
[Korean] 나는 석유 탐사개발 회사에서 완결 설계와 석유 생산 데이터를 비교 분석하고 있습니다.
유정 간격, 수평구간 길이, 천공 클러스터 간격, 주입 물량, 프로판트 모래 주입량(파운드), 수압파쇄 단계 수를 포함한 10,000개 유정의 수압파쇄 완결 설계 데이터를 생성해주세요. 
이러한 유정들의 생산 데이터를 생성해주세요. 완결 설계와 생산량 사이에는 관계가 있어야 합니다. 
생산량은 대략적으로 쌍곡선 감소를 따르고 약간의 노이즈가 있어야 합니다. 
데이터 생성 후, 생산량을 극대화하기 위한 최적의 완결 설계 매개변수를 결정하는 분석을 수행해주세요. 
종합적인 보고서를 작성하고 시각화를 위해 렌더링해주세요.`,
...
[Korean] 전기 및 수도 스마트 홈 미터를 기반으로 분석, 보고, 조치 권장사항을 제공하는 데모를 만들어주세요. 
